...
"""Sampler for Gemma transformer with adaptive temperature support."""
...
# New: Default polynomial coefficients for adaptive temperature
DEFAULT_POLY_COEFFS = [-0.037, 0.481, -2.3, 4.917, -1.791]
...
# Modified: Added temperature parameters to sampling state
@chex.dataclass
class _SamplingState:
    ...
    # New: Adaptive temperature parameters
    use_adaptive_temp: bool = False
    temp_poly_coeffs: jnp.ndarray | None = None
...
class Sampler:
    ...
    def __init__(
        ...
        use_adaptive_temp: bool = False,  # New: adaptive temperature flag
        temp_poly_coeffs: Sequence[float] | None = None,  # New: custom coefficients
    ):
        ...
        Args:
            ...
            use_adaptive_temp: whether to use adaptive temperature scaling.
            temp_poly_coeffs: optional custom coefficients for temperature polynomial.
        """
        self.transformer = transformer
        self.vocab = vocab
        self.params = params
        self.use_adaptive_temp = use_adaptive_temp
        self.temp_poly_coeffs = jnp.array(
            temp_poly_coeffs if temp_poly_coeffs is not None else DEFAULT_POLY_COEFFS
        )
    ...
    # New: Adaptive temperature computation
    def _compute_adaptive_temperature(self, logits: jax.Array) -> jax.Array:
        """Computes adaptive temperature scaling based on entropy.

        Args:
            logits: input logits [B, 1, V]

        Returns:
            scaled logits with same shape as input
        """
        original_probs = jax.nn.softmax(logits, axis=-1)
        entropy = -jnp.sum(
            original_probs * jnp.log(original_probs + 1e-9),
            axis=-1,
            keepdims=True
        )
        beta = jnp.where(
            entropy > 0.5,
            jnp.maximum(jnp.polyval(self.temp_poly_coeffs, entropy), 1.0),
            1.0
        )
        return logits * beta
    ...
    def _sample_step(
        ...
        # New: Apply adaptive temperature if enabled
        if self.use_adaptive_temp:
            logits = self._compute_adaptive_temperature(logits)
        ...
        return _SamplingState(
            ...
            use_adaptive_temp=sampler_state.use_adaptive_temp,  # New
            temp_poly_coeffs=sampler_state.temp_poly_coeffs,  # New
        )
    ...
    def init_sample_state(
        ...
        return _SamplingState(
            ...
            use_adaptive_temp=self.use_adaptive_temp,  # New
            temp_poly_coeffs=self.temp_poly_coeffs,  # New
        )
